...
function onConfirmWhenAddEdit() {
	$.ajax({
		...
		"data" : $('#t_servicetype_form').serialize() + '&jUserId=' + userId + '&language=' + engbId,
...
function onConfirmWhenDelete() {
	$.ajax({
		"type" : "POST",
		"url" : baseUrl + "stage_one_datasourse.php",
		"data" : 'action=deleteServiceData&RecordId=' + RecordId+ '&jUserId=' + userId + '&language=' + engbId,
		"success" : function(response) {
			$msgType = JSON.parse(response)['msgType'];
			$msg = JSON.parse(response)['msg'];
			if ($msgType == "success") {
...
jQuery('#t_servicetype_form').parsley({
	listeners : {
...
$(function() {
	...
	ServiceTable = $('#ServiceTable').dataTable({
		...
		"fnDrawCallback" : function(oSettings) {
			...
			$('a.itmEdit', ServiceTable.fnGetNodes()).each(function() {
				$(this).click(function() {
					var nTr = this.parentNode.parentNode;
					var aData = ServiceTable.fnGetData(nTr);
                    RecordId = aData[0];
                    $('#RecordId').val(aData[0]);
                    $('#STypeName').val(aData[2]);
                    $('#STypeNameFrench').val(aData[3]);
					msg = "Do you really want to edit this record?";
					onCustomModal(msg, "onEditPanel");
				});
			});
			$('a.itmDrop', ServiceTable.fnGetNodes()).each(function() {
				$(this).click(function() {
					var nTr = this.parentNode.parentNode;
					var aData = ServiceTable.fnGetData(nTr);
					RecordId = aData[0];
			...
			$('a.itmMore', ServiceTable.fnGetNodes()).each(function() {
				$(this).click(function() {
					var nTr = this.parentNode.parentNode;
