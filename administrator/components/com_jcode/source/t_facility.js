...
function onFormPanel() {
    ...
    if ($('#ACountryId').val() == '') {
       ...
       //$('#RegionId').val($('#ARegionId').val());
		...
		var selectedRegion = $('#ARegionId').val();
		if(selectedRegion==0)
			selectedRegion='';
		...
		getFillRegionEdit(selectedRegion,selectedDistrict);
...
function getFillRegionEdit(selectedRegion,selectedDistrict) {
    $.ajax({
        type: "POST",
        dataType: "json",
        url: baseUrl + "combo_generic.php",
        data: {
            "operation": 'getFillRegionForEdit',
            "CountryId": $('#ACountryId').val(),
            "UserId": userName,
            "lan": lan
        },
        success: function(response) {
			...
			getFillDistrictEditPanel(selectedDistrict);
        }
    });
}

function getFillDistrictEditPanel(selectedValue) {
	$.ajax({
		type : "POST",
		dataType : "json",
		url : baseUrl + "combo_generic.php",
		data : {
			"operation" : 'getFillDistrictForEdit',
			"CountryId": $('#ACountryId').val(),
		...
		success : function(response) {	
			...
			$('#ADistrict-list').html(html);
			$('#ADistrict-list').val(selectedValue);
            ...
            "fnDrawCallback": function(oSettings) {
                ...
                $('a.itmEdit', facilityTable.fnGetNodes()).each(function() {
                    $(this).click(function() {
						...
						getFillRegionEdit(aData[23],aData[27]);
						//getFillDistrictEditPanel(aData[27]);
