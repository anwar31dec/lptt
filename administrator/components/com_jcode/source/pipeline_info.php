...
<div class="container">
	<div class="content_fullwidth lessmar">
		...
		<div class="panel-heading clearfix">
			<?php echo $TEXT['National Stock Pipeline Information List']; ?>
		...
		<div class="panel-body">
			<div class="clearfix list-panel" id="tbl-pf">
				<table  id="tbl-facility-sum-list" class="table table-striped table-bordered display table-hover" cellspacing="0">
				   <thead></thead>
				   <tbody></tbody>
				</table>				
			</div>
...
<style>
	...
	#tbl-facility-sum-list  tr  th {
    vertical-align: middle !important;
	}
	
	#tbl-facility-sum-list td{
	border: 1px solid #e4e4e4 !important;
	text-align: right !important;	
	}
	#tbl-facility-sum-list th{
	border: 1px solid #e4e4e4 !important;
	
	}
		
	#tbl-facility-sum-list td:nth-child(2) {
		text-align: left !important;
	}
	
	#tbl-facility-sum-list td:nth-child(1) {
		text-align: center !important;
	}
	...
	#tbl-facility-sum-list .productname{
...
function print_function(type){
	var tableId = 'tbl-facility-sum-list';
	var reportSaveName = 'National_Stock_Pipeline_Info'; //Not allow any type of special character of cahrtName
	var reportHeaderList = new Array();
	var dataAlignment = new Array();
	var cellWidth = new Array();
	var chart = 1;

	var reportHeaderName = TEXT['National Stock Pipeline Information Report'];
	reportHeaderList[0] = reportHeaderName;
	reportHeaderList[1] = $('#country-list option[value='+$('#country-list').val()+']').text()+' - '+ $('#item-group-list option[value='+$('#item-group-list').val()+']').text()+ ' - ' + $('#OwnerType option[value='+$('#OwnerType').val()+']').text();
			...
			success: function(response) {
				...
				if(type == 'print'){
					...
					for(var m = 0; m < topThs.length; m++){
						htmlTable+= '<tr role="row" nobr="true" >';
...
//$('#genpdfReport').attr('href', baseUrl + "report/pdf_master_dynamic_column.php?action=pdfofGenericDatabase&curSearch=" + currentSearch);
...
function excel_function(){
   // var currentSearch = $('#tbl-facility-sum-list_filter').find('input').val();
   // currentSearch=currentSearch.replaceAll("+", "|");	
   var currentSearch='';
	window.open("<?php echo $baseUrl; ?>report/t_national_stock_pipeline_information_excel.php?jBaseUrl=<?php echo $jBaseUrl; ?>&lan=<?php echo $lan; ?>
	&MonthId="+$('#month-list').val()+"&YearId="+$('#year-list').val()+"&CountryId="+$('#country-list').val()+"&ItemGroupId="+$('#item-group-list').val()
...
<script src='<?php echo $baseUrl; ?>pipeline_info.js'></script>
