...
// 
	...
	function savePatientOverview(oGrid_event) {
		...
		Ext.Ajax.request({
			waitMsg : 'Please wait...',
			url : baseUrl + 't_national_level_monthlystatus_ext_server_view_view.php',
			params : {
	...
	function saveAdultRegimens(oGrid_event) {
		...
		Ext.Ajax.request({
			waitMsg : 'Please wait...',
			url : baseUrl + 't_national_level_monthlystatus_ext_server_view.php',
			params : {
	...
	var dsItemGroup = new Ext.data.Store({
		reader : new Ext.data.JsonReader({
			fields : ['ItemGroupId', 'GroupName', 'FrequencyId', 'StartMonthId', 'StartYearId'],
			root : 'rows'
		}),
	...
	dsMasterStockData.on('load', function(store, records, options) {
		...
		if (store.getCount() == 1) {
			...
			var msgbox = Ext.Msg.show({
						msg : 'You (' + vFacility + ') don\'t have the report on ' + vMonth + '-' + vYear ,
						icon : Ext.Msg.INFO,
						minWidth : 200,
						buttons : Ext.Msg.OK,
						scope : this
	...
	var cboCountry = new Ext.form.ComboBox({
		...
		listeners : {
			select : function(field, rec, selIndex) {				
				...
				dsItemGroup.removeAll(true);
				Ext.getCmp('cboItemGroupId').reset();

				dsMonth.removeAll(true);
				Ext.getCmp('cboMonthId').reset();
	...
	var cboItemGroup = new Ext.form.ComboBox({
		...
		listeners : {
			select : function(field, rec, selIndex) {
				...
				// //alert(rec.get('FrequencyId'));
				// resetStoreAndGrid();
				// // if (pItemGroupId != rec.get('ItemGroupId'))
				// // Ext.getCmp('cboFacilityId').reset();
// 				
				// initMonthId = rec.get('StartMonthId');
				// initYearId = rec.get('StartYearId');
// 
				// startDate = new Date(initYearId, initMonthId - 1);
				// startManthName = startDate.getMonthName();
// 				
				...
				// vItemId = pItemGroupId;
				// vItem = rec.get('GroupName');
				// pFrequencyId = rec.get('FrequencyId');
// 				
				...
				// // dsMasterStockData.setBaseParam('pItemGroupId', pItemGroupId);
// 
				// //pMonthId = Ext.getCmp('cboMonthId').getValue() == '' ? 0 : Ext.getCmp('cboMonthId').getValue();
// 								
				// pMonthId = 0;
				// pFacilityId = 0;
				// gRowIndex = 0;
// 				
				...
				dsMonth.load({
					params : {
						'FrequencyId' : rec.get('FrequencyId')
					}
				});
			}
		}
