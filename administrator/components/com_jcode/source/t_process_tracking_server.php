...
function insertUpdateProcessTracking($conn) {
    ...
    switch ($ProcessId) {
        ...
        case 2:
            ...
            if ($OptUnit == '1') {
                $subTrackNo = substr($TrackingNo, 0, 4);
                if ($subTrackNo == 'BGDA') {
                    echo json_encode(array('msgType' => 'error', 'msg' => 'This is not a textile job.'));
                    return;
                }
            }
            ...
            if ($OptUnit == '2') {
                $subTrackNo = substr($TrackingNo, 0, 4);
                if ($subTrackNo != 'BGDA') {
                    echo json_encode(array('msgType' => 'error', 'msg' => 'This is not a analytical job.'));
                    return;
                }
            } */
			
			if(patn_array_match($aTextilePrefix, $TrackingNo)){
				$ProcUnitId = 1;
			} else if (patn_array_match($aAnalyticalPrefix, $TrackingNo)){
				$ProcUnitId = 2;
			} else {
				 echo json_encode(array('msgType' => 'error', 'msg' => 'This is a invalid job not match with prefix.'));
                 return;
			}
            ...
            $aQuery1 = getProTrackInsertCmd1and2($TrackingNo, $ProcessId, $jUserId, $ProcUnitId);
            $aQuerys[] = $aQuery1;
...
function getProTrackInsertCmd1and2($TrackingNo, $ProcessId, $jUserId, $ProcUnitId) {
    if ($ProcUnitId == '2') {
        $ProcessId = 23;
    }
