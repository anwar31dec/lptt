...
function getProcessList() {
    ...
    $sWhere = "";
   /* if ($_POST['sSearch'] != "") {
        $sWhere = " WHERE (ProcessName like '%" . mysql_real_escape_string($_POST['sSearch']) . "%' )";
    }*/
    ...
    while ($row = @mysql_fetch_object($pacrs)) {
        ...
        echo '["' . $MapId . '", "' . getcheckBox($chkValue) . " " . $ProcessName . '", "' . $ProcessId . '"]';
...
function insertAllorOneMapping() {
    ...
    if ($checkVal == "true") {
        $sql = "INSERT INTO t_user_process_map (UserId, ProcessId) 
                VALUES ('" . $userName . "', '" . $ProcessId . "') ";
        ...
        $sql = "DELETE FROM t_user_process_map WHERE MapId = '" . $MapId . "' ";

        $aQuery1 = array('command' => 'DELETE', 'query' => $sql, 'sTable' => 't_user_process_map', 'pks' => array('MapId'), 'pk_values' => array($MapId), 'bUseInsetId' => FALSE);
        $aQuerys = array($aQuery1);
    }
