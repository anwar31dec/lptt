...
function onConfirmWhenAddEdit() {
	$.ajax({
		...
		"data" : $('#t_reportby_form').serialize() + '&jUserId=' + userId + '&language=' + engbId,
		"success" : function(response) {
			$msgType = JSON.parse(response)['msgType'];
			$msg = JSON.parse(response)['msg'];
			if ($msgType == "success") {
				ReportByTable.fnDraw();
				onSuccessMsg($msg);
				onListPanel();
			} else {
				onErrorMsg($msg);
...
function onConfirmWhenDelete() {
	$.ajax({
		"type" : "POST",
		"url" : baseUrl + "stage_one_datasourse.php",
		"data" : 'action=deleteReportByData&RecordId=' + RecordId + '&jUserId=' + userId + '&language=' + engbId,
		"success" : function(response) {
			$msgType = JSON.parse(response)['msgType'];
			$msg = JSON.parse(response)['msg'];
			if ($msgType == "success") {
...
jQuery('#t_reportby_form').parsley({
	listeners : {
...
$(function() {
	...
	ReportByTable = $('#ReportByTable').dataTable({
		...
		"fnDrawCallback" : function(oSettings) {
			...
			$('a.itmEdit', ReportByTable.fnGetNodes()).each(function() {
				$(this).click(function() {
					var nTr = this.parentNode.parentNode;
					var aData = ReportByTable.fnGetData(nTr);
                    RecordId = aData[0];
			...
			$('a.itmDrop', ReportByTable.fnGetNodes()).each(function() {
				$(this).click(function() {
					var nTr = this.parentNode.parentNode;
					var aData = ReportByTable.fnGetData(nTr);
					RecordId = aData[0];
			...
			$('a.itmMore', ReportByTable.fnGetNodes()).each(function() {
				$(this).click(function() {
					var nTr = this.parentNode.parentNode;
					if ($(this).children('span').attr('class') == 'label label-info faminus') {
						...
						ReportByTable.fnOpen(nTr, fnFormatDetails(nTr), 'details');
					}
				});
			});
		},
