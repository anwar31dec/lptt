...
function onConfirmWhenAddEdit() {
	$.ajax({
		...
		"data" : $('#t_Service_Area_form').serialize() + '&jUserId=' + userId + '&language=' + engbId,
		"success" : function(response) {
			$msgType = JSON.parse(response)['msgType'];
			$msg = JSON.parse(response)['msg'];
			if ($msgType == "success") {
				ServiceAreaTable.fnDraw();
				onSuccessMsg($msg);
				onListPanel();
			} else {
				onErrorMsg($msg);
...
function onConfirmWhenDelete() {
	$.ajax({
		"type" : "POST",
		"url" : baseUrl + "stage_one_datasourse.php",
		"data" : 'action=deleteServiceAreaData&RecordId=' + RecordId + '&jUserId=' + userId + '&language=' + engbId,
		"success" : function(response) {
			$msgType = JSON.parse(response)['msgType'];
			$msg = JSON.parse(response)['msg'];
			if ($msgType == "success") {
...
jQuery('#t_Service_Area_form').parsley({
	listeners : {
...
$(function() {
	...
	ServiceAreaTable = $('#ServiceAreaTable').dataTable({
		...
		"fnDrawCallback" : function(oSettings) {
			...
			$('a.itmEdit', ServiceAreaTable.fnGetNodes()).each(function() {
				$(this).click(function() {
					var nTr = this.parentNode.parentNode;
					var aData = ServiceAreaTable.fnGetData(nTr);
                    RecordId = aData[0];
                    $('#RecordId').val(aData[0]);
                    $('#ServiceAreaName').val(aData[2]);
                    $('#ServiceAreaNameFrench').val(aData[3]);
					msg = "Do you really want to edit this record?";
					onCustomModal(msg, "onEditPanel");
				});
			});
			$('a.itmDrop', ServiceAreaTable.fnGetNodes()).each(function() {
				$(this).click(function() {
					var nTr = this.parentNode.parentNode;
					var aData = ServiceAreaTable.fnGetData(nTr);
					RecordId = aData[0];
			...
			$('a.itmMore', ServiceAreaTable.fnGetNodes()).each(function() {
				$(this).click(function() {
					var nTr = this.parentNode.parentNode;
					if ($(this).children('span').attr('class') == 'label label-info faminus') {
						...
						ServiceAreaTable.fnOpen(nTr, fnFormatDetails(nTr), 'details');
					}
				});
			});
		},
