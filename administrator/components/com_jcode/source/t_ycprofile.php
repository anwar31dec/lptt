<?php 
$user = JFactory::getUser();
$baseUrl = JURI::base() . 'administrator/components/com_jcode/source/';
$jBaseUrl = JURI::base();
$lang = JFactory::getLanguage();
$lan = $lang->getTag();
$userName = $user->username;
$jUserId = $user->name;
?>

<script>
    var baseUrl = '<?php echo $baseUrl; ?>';
    var lan ='<?php echo $lan;?>'; 
    var userid ='<?php echo $userName;?>'; 
    var jUserId ='<?php echo $jUserId;?>'; 
</script>


<?php 
    include_once ('database_conn.php');
	include_once ('function_lib.php');
	include_once ('init_month_year.php');	
	include_once ('combo_script.php');
	include_once ('language/lang_en.php');
	include_once ('language/lang_fr.php');
	include_once ('language/lang_switcher.php');
?>

<div class="page-title">
	<h3 class="no-margin"><?php echo $TEXT['Country Profile Entry']; ?></h3>
    <span class="pull-right" id="Btn">
   	    <button class="btn btn-info" type="button" id="PrintBTN" onclick="print_function()" > <?php echo $TEXT['Print']; ?> </button>
        <button class="btn btn-info" type="button" id="PrintBTN1" onclick="excel_function()" > <?php echo $TEXT['Excel']; ?> </button>
    </span>
</div>

<br />
<br />
<div class="row">
	<div class="col-md-12 col-sm-12 col-sx-12">
		<div class="panel panel-default">
			<div class="panel-body">
				<div class="center-block" id="month-year-block" >
					<table id="month-year">
						<tbody>
							<tr>

								<td><?php echo $TEXT['Year']; ?>&nbsp;:&nbsp;&nbsp;</td><td valign="middle" align="left">
								<select class="form-control" name="Year" id="Year">
								</select></select></td>	
								
								<td>&nbsp;&nbsp;&nbsp;</td>
								<td><?php echo $TEXT['Country']; ?>&nbsp;:&nbsp;&nbsp;</td><td valign="middle" align="left">
								<select class="form-control chzn-select" name="CountryName" id="CountryName"
								onchange="countryOnchange(this.value)"></select>							
									
								<td>&nbsp;&nbsp;&nbsp;</td>
								<td><?php echo $TEXT['Product Group']; ?>&nbsp;:&nbsp;&nbsp;</td>
								<td>
									<select class="form-control" id="item-group"></select>
								</td>
															
							</tr>
						</tbody>
					</table>
				</div>
			</div>
		</div>
	</div>
	
</div>

<div class="panel panel-default" >
	<div class="panel-body">
		<div class="clearfix list-panel country-form-wizard" >
			<div class="panel panel-default">
				<div class="form-horizontal no-margin form-border" id="country_form_wizard" novalidate>
					<div class="panel-heading">
						<?php echo $TEXT['Country Profile Wizard']; ?>
					</div>
					<div class="panel-tab">
						<ul class="wizard-steps wizard-demo" id="wizardDemo1">
							<li class="active">
								<a href="#wizardContent1" data-toggle="tab" onClick="onFirstTab()" ><?php echo $TEXT['Basic Information']; ?></a>
							</li>
							<li>
								<a href="#wizardContent2" data-toggle="tab" onClick="onSecondTab()"><?php echo $TEXT['Funding Source']; ?></a>
							</li>
							<li id="step3">
								<a href="#wizardContent3" data-toggle="tab" onClick="onThirdTab()"><?php echo $TEXT['Regimens/Patients']; ?></a>
							</li>
							<li>
								<a href="#wizardContent4" data-toggle="tab" onClick="onFourthTab()"><?php echo $TEXT['Funding Requirements']; ?></a>
							</li>
							<li>
								<a href="#wizardContent5" data-toggle="tab" onClick="onFifthTab()"><?php echo $TEXT['Pledged Funding']; ?></a>
							</li>
						</ul>
					</div>

					<div class="panel-body" id="table-div">
						<div class="tab-content">
							<div class="tab-pane fade in active" id="wizardContent1">								
								<a href="javascript:void(0);" class="btn btn-warning btn-sm" onClick="onClearWizardInformation(1)" style="float:right;"><?php echo $TEXT['Clear Basic Information']; ?></a>								
								<table  id="profileTable" class="table table-striped table-bordered display table-hover" cellspacing="0">
									<thead>
										<tr>
											<th><?php echo $TEXT['Profile Id']; ?></th>
											<th style="text-align: center;">SL#</th>
											<th><?php echo $TEXT['Parameter Name']; ?></th>
											<th style="text-align: center;"><?php echo $TEXT['Value']; ?></th>
											<th><?php echo $TEXT['Param Id']; ?></th>
										</tr>
									</thead>
									<tbody></tbody>
								</table>
							</div>
							
							<div class="tab-pane fade" id="wizardContent2">								
								<a href="javascript:void(0);" class="btn btn-warning btn-sm" onClick="onClearWizardInformation(2)" style="float:right; display:none;"><?php echo $TEXT['Funding Source']; ?></a>
								<p class='clearfix' style="padding:3px;">&nbsp;</p>
								<table  id="tbl-yc-funding-source-assign" class="table table-striped table-bordered display table-hover" cellspacing="0">	
									<thead>
										<tr>
										<th>Id</th>
										<th><?php echo $TEXT['Funding Source Name']; ?></th>															
										</tr>
									</thead>
									<tbody></tbody>
								</table>
							</div>
							
							<div class="tab-pane fade" id="wizardContent3">								
								<a href="javascript:void(0);" class="btn btn-warning btn-sm" onClick="onClearWizardInformation(3)" style="float:right;"><?php echo $TEXT['Clear Regimens/Patients']; ?></a>
								<p class='clearfix' style="padding:3px;">&nbsp;</p>
								<table  id="tbl-yc-regimen-patient" class="table table-striped table-bordered display table-hover" cellspacing="0">
									<thead>
										<tr>
											<th>SL</th>																							
											<th><?php echo $TEXT['Formulation']; ?></th>
											<th>(0-4 Years)</th>
											<th>(5-14 Years)</th>
											<th>(15+ Years)</th>
											<th>Pregnant women</th>											
										</tr>
									</thead>
									<tbody></tbody>
								</table>
							</div>
							<div class="tab-pane fade" id="wizardContent4">
								<a href="javascript:void(0);" class="btn btn-warning btn-sm" onClick="onClearWizardInformation(4)" style="float:right;"><?php echo $TEXT['Clear Funding Requirements']; ?></a>								
								<p class='clearfix' style="padding:3px;">&nbsp;</p>								
								<div class="alert alert-info">
									<!--Please enter all monetary values in Euro-->
									<?php echo $TEXT['MonetaryValues']; ?>
								</div>
								<table  id="tbl-yc-funding-requirement" class="table table-striped table-bordered display table-hover" cellspacing="0">
									<thead>
										<tr>
											<th>SL</th>
											<th>FundingReqId</th>
											<th><?php echo $TEXT['Product']; ?></th>											
											<th><?php echo $TEXT['Formulation']; ?></th>
											<th><span class="cYear">2013</span></th>
											<th><span class="nYear">2014</span></th>
											<th><span class="nnYear">2015</span></th>
											<th><?php echo $TEXT['Total']; ?></th>
										</tr>
									</thead>
									<tbody></tbody>
								</table>
							</div>
							<div class="tab-pane fade" id="wizardContent5">
								<a href="javascript:void(0);" class="btn btn-warning btn-sm" onClick="onClearWizardInformation(5)" style="float:right;"><?php echo $TEXT['Clear Pledged Funding']; ?></a>
								<p class='clearfix' style="padding:3px;">&nbsp;</p>
								<div class="alert alert-info">
									<!--Please enter all monetary values in Euro-->
									<?php echo $TEXT['MonetaryValues']; ?>
								</div>
								<div class="btn-group" style="float:right;">
									<button class="btn btn-default pf" onClick="onRequirementYear(1)" type="button">2013</button>
									<button class="btn btn-default pf" onClick="onRequirementYear(2)" type="button">2013</button>
									<button class="btn btn-default pf" onClick="onRequirementYear(3)" type="button">2013</button>
								</div>
								<!--
								<a href="javascript:void(0);" class="btn btn- btn-sm pf" onClick="onRequirementYear(3)" style="float:right;"><span class="nnYear">2013</span></a>
								<a href="javascript:void(0);" class="btn btn- btn-sm pf" onClick="onRequirementYear(2)" style="float:right;margin-right:10px;"><span class="nYear">2013</span></a>								
								<a href="javascript:void(0);" class="btn btn- btn-sm pf" onClick="onRequirementYear(1)" style="float:right;margin-right:10px;"><span class="cYear">2013</span></a>
								-->
								<p class='clearfix' style="padding:3px;">&nbsp;</p>
								<!--new rubel-->
								
								<div class="row" style="clear:both;">
									<div class="col-md-12" id="list-panel7">
										
										<div id="tbl-block1">
											<table class="table table-striped display" id="ycpledgedfunding" style="width:100%">
												<thead>								
												</thead>
												<tbody>
												</tbody>
											</table>
										</div>
										
							
									</div>
								</div>
								<!--new rubel-->
								
								
							<!--	<div id="yc_pledged_funding"></div>-->
							</div>
						</div>
					</div>
					<div class="panel-footer clearfix">
						<div class="pull-left">
							<button class="btn btn-success btn-sm disabled" id="prevStep1" disabled>
								<?php echo $TEXT['Previous']; ?>
							</button>
							<button type="submit" class="btn btn-sm btn-success" id="nextStep1">
								<?php echo $TEXT['Next']; ?>
							</button>
						</div>

						<div class="pull-right" style="width:30%">
							<div class="progress progress-striped active m-top-sm m-bottom-none">
								<div class="progress-bar progress-bar-success" id="wizardProgress" style="width:25%;"></div>
							</div>
						</div>
					</div>
				</div>
			</div><!-- /panel -->

		</div>
	</div>
</div>

<style>
	.SL, .Value {
		text-align: center !important;
	}
	.right-aln {
		text-align: right !important;
	}
	.left-aln {
		text-align: left !important;
	}
	.center-aln {
		text-align: center !important;
	}
	
	.datacell, .right-aln {
		text-align: right !important;
	}
	.datacell.yc_11, .datacell.yc_12 {
		text-align: left;
	}
	input[type="checkbox"], input[type="radio"]{
		opacity:1 !important;
	}
	.multiselect{
		float:left;
	}
	.htCore .htDimmed{
		text-align:left !important;
	}
	.htCore .htNumeric, .htCore .htDimmed:last-child {
		text-align:right !important;
	}
	#ycpledgedfunding input, textarea, .uneditable-input {
		width: 100% !important;
	}
	#ycpledgedfunding .DataTables_sort_wrapper{
	text-align: center !important;
	}
</style>

<script>
function print_function(){
	window.open("<?php echo $baseUrl; ?>report/t_ycprofile_print.php?jBaseUrl=<?php echo $jBaseUrl; ?>&lan=<?php echo $lan;?>
	&CountryId="+$('#CountryName').val()
    +"&Year="+$('#Year').val()
	+"&ItemGroupId="+$('#item-group').val()
    +"&RequirementYear="+RequirementYear
    +"&CountryName="+$('#CountryName option[value='+$('#CountryName').val()+']').text());			 
} 		
function excel_function(){
	window.open("<?php echo $baseUrl; ?>report/t_ycprofile_excel.php?jBaseUrl=<?php echo $jBaseUrl; ?>&lan=<?php echo $lan;?>
	&CountryId="+$('#CountryName').val()
    +"&Year="+$('#Year').val()
	+"&ItemGroupId="+$('#item-group').val()
    +"&RequirementYear="+RequirementYear
    +"&CountryName="+$('#CountryName option[value='+$('#CountryName').val()+']').text());		  
} 		
</script>

<link href="<?php echo $baseUrl; ?>css/endless.min.css" rel="stylesheet"/>
<link href="<?php echo $baseUrl; ?>media/datatable-bootstrap/dataTables.bootstrap.css" rel="stylesheet"/>
<link href="<?php echo $baseUrl; ?>css/jquery.dataTables.custom.css" rel="stylesheet"/>
<script src='<?php echo $baseUrl; ?>media/datatable/js/jquery.dataTables.min.js'></script>
<script src='<?php echo $baseUrl; ?>media/datatable-bootstrap/dataTables.bootstrap.min.js'></script>

<script src='<?php echo $baseUrl; ?>t_ycprofile.js'></script>

