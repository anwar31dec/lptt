			...
			FROM (SELECT
			  ...
			  LEFT JOIN (SELECT
			             ...
			             WHERE t_cfm_masterstockstatus.Year = '$year'
			                 ...
			                 AND t_cfm_masterstockstatus.CountryId = $countryId) b
...
function getPatientOverview() {
	global $facilityId, $monthId, $year, $reportId, $language;
	
	$patientTypeName = '';
	
	if($language == ENGLISH)
		$patientTypeName = 'FormulationName';
	else if($language == FRENCH)
		$patientTypeName = 'FormulationNameFrench';
	...
	$query .= " ORDER BY a.FormulationName desc";
...
function getRegimens() {
	...
	global $facilityId, $monthId, $year, $formulationId, $reportId, $countryId, $language;
	...
	foreach ($formulationIds as $formulationId) {
		...
		foreach ($rows as $row) {
			if ($formulationId == $row['FormulationId']) {
				...
				$total = $total + $row['RefillPatient'];
			}
		}
		$datum['TotalPatient'] = $total;
		$data[] = $datum;
	}
...
function changeBsubmittedInMaster() {
	...
	updatePatientOverviewFromRegimen();
...
function updatePatientOverviewFromRegimen() {
	
	global 	$facilityId, $monthId, $year, $countryId, $itemGroupId, $reportId;	
		
	$query = "SELECT
			  t_regimen.FormulationId,
			  SUM(t_cfm_regimenpatient.RefillPatient)    RefillPatient,
			  SUM(t_cfm_regimenpatient.NewPatient)    NewPatient,
			  SUM(t_cfm_regimenpatient.TotalPatient)    TotalPatient
			FROM t_cfm_regimenpatient
			  INNER JOIN t_cfm_masterstockstatus
			    ON (t_cfm_regimenpatient.CFMStockId = t_cfm_masterstockstatus.CFMStockId)
			  INNER JOIN t_regimen
			    ON (t_cfm_regimenpatient.RegimenId = t_regimen.RegimenId)
			WHERE (t_cfm_masterstockstatus.Year = '$year'
			       AND t_cfm_masterstockstatus.MonthId = $monthId
			       AND t_cfm_masterstockstatus.CountryId = $countryId
			       AND t_cfm_masterstockstatus.FacilityId = $facilityId
				   AND t_cfm_masterstockstatus.CFMStockId = $reportId)
			GROUP BY t_regimen.FormulationId;";
	...
	try 
	{
		...
		while($rec = mysql_fetch_assoc($rResult)) {
				$sql = "UPDATE t_cfm_patientoverview SET RefillPatient = " . (is_null($rec['RefillPatient']) ? 'NULL':$rec['RefillPatient']) . ", NewPatient = " 
				. (is_null($rec['NewPatient']) ? 'NULL':$rec['NewPatient']). ', TotalPatient = ' . (is_null($rec['TotalPatient']) ? 'NULL':$rec['TotalPatient'])
				. " WHERE FormulationId = " .  $rec['FormulationId'] . " AND FacilityId = $facilityId AND MonthId = $monthId AND `Year` = $year
				AND CountryId = $countryId AND CFMStockId = $reportId;";
