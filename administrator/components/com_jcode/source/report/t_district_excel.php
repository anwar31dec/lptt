...
$jBaseUrl = $_GET['jBaseUrl'];
    ...
    if ($total>0){
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('A2',$gTEXT['District List']);
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('A3',$gTEXT['Country'].' : '.$CountryName);
		$styleThinBlackBorderOutline = array('borders' => array('outline'   => array('style' => PHPExcel_Style_Border::BORDER_THIN, 'color'  => array('argb' => 'FF000000') )));
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('A4',$gTEXT['Region'].' : '.$RegionName);
		$styleThinBlackBorderOutline = array('borders' => array('outline'   => array('style' => PHPExcel_Style_Border::BORDER_THIN, 'color'  => array('argb' => 'FF000000') )));
		$objPHPExcel -> getActiveSheet() -> getStyle('A4') -> getAlignment() -> setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$objPHPExcel->getActiveSheet()->getStyle('A4')->getFont()->setBold(true);
		$objPHPExcel -> getActiveSheet() -> duplicateStyleArray(array('font' => array('size' => '11', 'bold' => true)), 'A4');						
		$objPHPExcel -> getActiveSheet() -> mergeCells('A4:B4');												
		$objPHPExcel->getActiveSheet()
									->SetCellValue('A6', 'SL#')							
					                ->SetCellValue('B6', $gTEXT['District Name']);
		...
		while($rec=mysql_fetch_array($r)){
            ...
            $objPHPExcel->getActiveSheet()->SetCellValue('A'.$j, $i)							
            						      ->SetCellValue('B'.$j, $rec['DistrictName']); 
