...
$jBaseUrl = $_GET['jBaseUrl']; 
    $sWhere = "";
	if ($_GET['curSearch'] != "") {
		$sWhere = " WHERE  (PAgencyName LIKE '%" . mysql_real_escape_string($_GET['curSearch']) . "%') ";
	}


    $sql=" SELECT  PAgencyId, PAgencyName	
				FROM t_procurement_agents". $sWhere." order by PAgencyId";
    ...
    if ($total>0){
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('A2', $gTEXT[ 'Procuring Agents List'])	;
	    ...
	    $objPHPExcel->getActiveSheet()
									->SetCellValue('A6', 'SL#')							
									->SetCellValue('B6', $gTEXT[ 'Procuring Agents']);
	...
	while($rec=mysql_fetch_array($r)){
		...
		$objPHPExcel->getActiveSheet()
									->SetCellValue('A'.$j, $i)							
								    ->SetCellValue('B'.$j, $rec['PAgencyName']);
	...
	$file = 'Procuring_Agents_List_'.$exportTime. '.xlsx';
