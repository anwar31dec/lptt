...
$jBaseUrl = $_GET['jBaseUrl'];
    ...
    $sWhere = "";
	if ($_GET['sSearch'] != "") {
		$sWhere = " WHERE  (OwnerTypeName LIKE '%" . mysql_real_escape_string($_GET['sSearch']) . "%'
                    OR " . " OwnerTypeNameFrench LIKE '%" . mysql_real_escape_string($_GET['sSearch']) . "%') ";
	}
	...
	if ($total>0){
		...
		echo '<!DOCTYPE html>
    		...
    		<tr>
				...
				<th style="text-align: left;">'.$gTEXT['Owner Type Name'].'</th> 
				<th style="text-align: left;">'.$gTEXT['Owner Type Name(French)'].'</th>
		    </tr>';
	while($rec=mysql_fetch_array($r)){
			echo '<tr>
		 	      <td style="text-align: center;">'.$i.'</td>
			      <td style="text-align: left;">'.$rec['OwnerTypeName'].'</td>
			      <td style="text-align: left;">'.$rec['OwnerTypeNameFrench'].'</td>
			      </tr>';
