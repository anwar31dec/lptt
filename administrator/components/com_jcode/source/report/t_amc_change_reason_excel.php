...
$jBaseUrl = $_GET['jBaseUrl']; 
 	...
 	$sWhere = "";
	if ($_GET['curSearch'] != "") {
		$sWhere = " WHERE  (AmcChangeReasonName LIKE '%" . mysql_real_escape_string($_GET['curSearch']) . "%') ";
	}
    $sql=" SELECT  AmcChangeReasonId,AmcChangeReasonName
         FROM t_amc_change_reason".$sWhere."";
    ...
    if ($total>0){
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('A2',$gTEXT['AMC Change Reason List'])	;
		...
		$objPHPExcel->getActiveSheet()		
									->SetCellValue('A6', 'SL#')							
								     ->SetCellValue('B6', $gTEXT['AMC Change Reason Name']);
	   ...
	   while($rec=mysql_fetch_array($r)){
			...
			$objPHPExcel->getActiveSheet()
									->SetCellValue('A'.$j, $i)							
									->SetCellValue('B'.$j, $rec['AmcChangeReasonName']);
	...
	$file = 'AMC_Change_Reason_'.$exportTime. '.xlsx';
