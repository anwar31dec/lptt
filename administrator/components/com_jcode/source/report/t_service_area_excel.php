...
$jBaseUrl = $_GET['jBaseUrl'];
    ...
    if ($total>0){
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('A2',$gTEXT['Service Area'])	;
		...
		$objPHPExcel->getActiveSheet()
									->SetCellValue('A6', 'SL#')							
					                ->SetCellValue('B6', $gTEXT['Service Area Name'])
									->SetCellValue('C6', $gTEXT['Service Area Name (French)']);
		...
		while($rec=mysql_fetch_array($r)){
            ...
            $objPHPExcel->getActiveSheet()->SetCellValue('A'.$j, $i)							
            						      ->SetCellValue('B'.$j, $rec['ServiceAreaName'])
										  ->SetCellValue('C'.$j,html_entity_decode($rec['ServiceAreaNameFrench'], ENT_QUOTES, "UTF-8")); 
	...
	$file = 'Service_Area_'.$exportTime. '.xlsx';
