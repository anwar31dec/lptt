...
$jBaseUrl = $_GET['jBaseUrl'];
    $sWhere = "";
	if ($_GET['curSearch'] != "") {
		$sWhere = " WHERE  (ShipmentStatusDesc LIKE '%" . mysql_real_escape_string($_GET['curSearch']) . "%') ";
	}
    $sql=" SELECT  ShipmentStatusId, ShipmentStatusDesc	
		   FROM t_shipmentstatus".$sWhere." order by ShipmentStatusId";
	...
	if ($total>0){
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('A2',$gTEXT['Shipment Status List']);
	    ...
	    $objPHPExcel->getActiveSheet()
									->SetCellValue('A6', 'SL#')							
									->SetCellValue('B6', $gTEXT['Shipment Status']);
     ...
     while($rec=mysql_fetch_array($r)){
        $objPHPExcel->getActiveSheet()
									->SetCellValue('A'.$j, $i)							
									->SetCellValue('B'.$j, $rec['ShipmentStatusDesc']); 
	...
	$file = 'Shipment_Status_'.$exportTime. '.xlsx';
