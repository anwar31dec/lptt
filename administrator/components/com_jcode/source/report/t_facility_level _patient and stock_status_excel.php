...
$query = "SELECT CFMStockId, FacilityId, MonthId, Year, ItemGroupId, 
	...
	$query .= " WHERE FacilityId =  $facilityId  and MonthId =  $monthId and Year = '$year' and ItemGroupId =  $ItemGroupId AND CountryId = $CountryId";
...
//****************************************************************************************    	
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue('A2',$gTEXT['Facility Level Patient And Stock Status']);
	...
	while($rec=mysql_fetch_array($r)){
        ...
        $objPHPExcel->getActiveSheet()
        		->SetCellValue('A'.$j, $rec['CFMStockId'])							
...
//****************************************Patient Overview *********************************//	
   ...
   $query_1 = "SELECT 	b.CFMPOId,
	...
	$query_1 .= "  ORDER BY b.CFMPOId,FormulationName  "; 
	 mysql_query("SET character_set_results=utf8");                
	...
	while ($rec_1 = mysql_fetch_array($r_1)) {
        ...
        $objPHPExcel->getActiveSheet()
        						...
        						->SetCellValue('C'.$j, $rec_1['RefillPatient']);
								//->SetCellValue('D'.$j, $rec_1['NewPatient'])								
        						//->SetCellValue('E'.$j, $rec_1['TotalPatient']);
		$objPHPExcel -> getActiveSheet() -> getStyle('C'.$j) -> getAlignment() -> setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
        //$objPHPExcel -> getActiveSheet() -> getStyle('D'.$j) -> getAlignment() -> setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
	    //$objPHPExcel -> getActiveSheet() -> getStyle('E'.$j) -> getAlignment() -> setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT); 
...
//************************************************Patient By Regimen*********************************
   ...
   // $objPHPExcel -> getActiveSheet() -> getStyle('D'.$j) -> getAlignment() -> setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
	...
	$query_2 = "SELECT b.CFMPatientStatusId, a.RegimenName, c.FormulationName, b.RefillPatient, b.NewPatient, b.TotalPatient 
			FROM t_regimen a 
	...
	$query_2 .= "and FacilityId =  $facilityId  and MonthId =  $monthId and Year = '$year' AND CountryId = $CountryId AND b.ItemGroupId = $ItemGroupId";
	$query_2 .= " order by FormulationName,RegimenName, b.CFMPatientStatusId desc";
	 mysql_query("SET character_set_results=utf8"); 
			  		 ...
			  		 if($tempGroupId!=$rec_2['FormulationName']) 
		   {
			...
			$tempGroupId=$rec_2['FormulationName'];$j++;
	 		...
	 		$objPHPExcel->getActiveSheet()
									...
									->SetCellValue('C'.$j, $rec_2['RefillPatient'])	;								
									//->SetCellValue('D'.$j, $rec_2['NewPatient'])									
									//->SetCellValue('E'.$j, $rec_2['TotalPatient']);
...
//********************************************Stock Status*****************************************************  
    ...
    $objPHPExcel -> getActiveSheet() -> mergeCells('A'.$j.':P'.$j);	
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue('A'.$j, 'SL#')							
								  ...
								  ->SetCellValue('N'.$j, $gTEXT['Max Qty'])
								  ->SetCellValue('O'.$j, $gTEXT['Order Qty'])
            					  ->SetCellValue('P'.$j, $gTEXT['Actual Order Qty']);
	...
	$objPHPExcel -> getActiveSheet() -> duplicateStyleArray(array('font' => array('size' => '12', 'bold' => true)), 'N'.$j);
	$objPHPExcel -> getActiveSheet() -> duplicateStyleArray(array('font' => array('size' => '12', 'bold' => true)), 'O'.$j);
	$objPHPExcel -> getActiveSheet() -> duplicateStyleArray(array('font' => array('size' => '12', 'bold' => true)), 'P'.$j);
	...
	$objPHPExcel -> getActiveSheet() -> getStyle('N'.$j) -> getAlignment() -> setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
    $objPHPExcel -> getActiveSheet() -> getStyle('O'.$j) -> getAlignment() -> setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
	$objPHPExcel -> getActiveSheet() -> getStyle('P'.$j) -> getAlignment() -> setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT); 
    ...
    $query_3 = "SELECT a.CFMStockStatusId, a.FacilityId, a.MonthId, a.Year, a.ItemNo, b.ItemName, a.OpStock OpStock_A, 0 OpStock_C, a.ReceiveQty, a.DispenseQty, 
	...
	while ($rec_3 = mysql_fetch_array($r_3)) {
	 		...
	 		$objPHPExcel->getActiveSheet()
									...
									->SetCellValue('I'.$j, $rec_3['ClStock_A'])
									->SetCellValue('J'.$j, $rec_3['SourceName'])								
									->SetCellValue('K'.$j, $rec_3['AMC'])									
									->SetCellValue('L'.$j, $rec_3['AmcChangeReasonId'])									
									->SetCellValue('M'.$j, ($rec_3['MOS']==0? '':number_format(($rec_3['MOS']),1))) 
				                    ->SetCellValue('N'.$j, $rec_3['MaxQty']==''? '':number_format($rec_3['MaxQty']))									
									->SetCellValue('O'.$j, $rec_3['OrderQty']==''? '':number_format($rec_3['OrderQty']))									
									->SetCellValue('P'.$j, $rec_3['ActualQty']==''? '':number_format($rec_3['ActualQty'])); 
		...
		$objPHPExcel -> getActiveSheet() -> getStyle('P' . $j . ':P' . $j) -> getAlignment() -> setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);	
