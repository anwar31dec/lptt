...
function getSummaryChart(){
	...
	if($ItemGroupId > 0){ 
		...
		$sql =" SELECT a.ItemNo, b.ItemName, SUM(DispenseQty) ReportedConsumption, SUM(ClStock) ReportedClosingBalance, 
			SUM(IF(d.FLevelId = 99,AMC,0)) AMC,
		IFNULL(((SUM(ClStock))/(SUM(IF(d.FLevelId = 99,AMC,0)))),0) MOS

			FROM t_cfm_stockstatus a 
			INNER JOIN t_itemlist b ON a.ItemNo = b.ItemNo AND b.bKeyItem = 1 AND b.ItemGroupId = ".$ItemGroupId."
			INNER JOIN t_cfm_masterstockstatus c ON a.CFMStockId = c.CFMStockId AND a.CountryId = c.CountryId AND c.StatusId = 5 
			INNER JOIN t_facility d ON a.FacilityId = d.FacilityId 
			AND (d.RegionId = ".$RegionId." OR ".$RegionId." = 0) AND (d.DistrictId = ".$DistrictId." OR ".$DistrictId." = 0)
