...
function getFacilityData($conn) {
    ...
    $sql = " SELECT SQL_CALC_FOUND_ROWS FacilityId, a.CountryId, a.RegionId, ParentFacilityId, 
             ...
             " . $OwnerTypeId . " " . $ServiceAreaId . " " . $FacilityType . " " . $FacilityLevel . " 
    ...
    while ($row = @mysql_fetch_object($pacrs)) {
        ...
        $PFacilityName = isset($r->PFacilityName)? $r->PFacilityName : '';
...
function insertUpdateFacilityData($conn) {
    ...
    $PFacilityId = isset($_POST['PFacilityId'])? $_POST['PFacilityId'] : '';
    if ($PFacilityId == 0)
        $PFacilityId = 'NULL';
    $RegionId = $_POST['RegionId'];
    $FLevelId = $_POST['FLevelId'];
    $FacilityCode = str_replace("'", "''", $_POST['FacilityCode']);  //$_POST['FacilityCode'];
    $FacilityName = str_replace("'", "''", $_POST['FacilityName']);  //$_POST['FacilityName'];
    $DistrictId = $_POST['ADistrict-list'];
    $OwnerTypeId = $_POST['AOwnerType'];
    $ServiceAreaId = $_POST['AServiceAreaId'];
    $FacilityAddress = str_replace("'", "''", $_POST['FacilityAddress']); //$_POST['FacilityAddress'];
    $FacilityPhone = str_replace("'", "''", $_POST['FacilityPhone']); //$_POST['FacilityPhone'];
    $FacilityFax = str_replace("'", "''", $_POST['FacilityFax']);  //$_POST['FacilityFax'];
    $FacilityEmail = str_replace("'", "''", $_POST['FacilityEmail']);  //$_POST['FacilityEmail'];
    $FacilityManager = isset($_POST['FacilityManager'])? $_POST['FacilityManager'] : '';
    $FacilityManager = str_replace("'", "''", $FacilityManager); //$_POST['FacilityManager'];
    //$FacilityFax = $_POST['FacilityFax'];
    $FacilityCount = isset($_POST['FacilityCount'])? $_POST['FacilityCount'] : '';
    ...
    $agentType = isset($_POST['AgentType'])? $_POST['AgentType'] : '';
    // if($agentType == 1){
    //      $agentType = 3;
    //  }
    //  else{
    ...
    if ($agentType == 'true' || $agentType == 'on') {
        $agentType = 3;
    } else {
        $agentType = 0;
    }
...
function getFacilityWarehouse($conn) {
    ...
    if ($total == 0)
        ...
        //array_unshift($arr, array('FacilityId' => '', 'FacilityName' => '[None]'));
...
function getFacilityGroupMap() {
    ...
    $iFilteredTotal =isset($aResultFilterTotal[0])?$aResultFilterTotal[0]:0;
    $iTotal = 0;
    if($rResult)
