...
function onConfirmWhenAddEdit() {
	$.ajax({
		...
		"data" : $('#t_shipment_form').serialize() + '&jUserId=' + userId + '&language=' + engbId,
...
function onConfirmWhenDelete() {
	$.ajax({
		"type" : "POST",
		"url" : baseUrl + "stage_one_datasourse.php",
		"data" : 'action=deleteSStatusData&RecordId=' + RecordId+ '&jUserId=' + userId + '&language=' + engbId,
		"success" : function(response) {
			$msgType = JSON.parse(response)['msgType'];
			$msg = JSON.parse(response)['msg'];
			if ($msgType == "success") {
...
jQuery('#t_shipment_form').parsley({
	listeners : {
...
$(function() {
	...
	$('#submitItemList').click(function() {
		$("#t_shipment_form").submit();
	});

	SStatusTable = $('#SStatusTable').dataTable({
		"bFilter" : true,		
		"bSort" : true,
		"bInfo" : true,
		"bPaginate" : true,
		...
		"fnDrawCallback" : function(oSettings) {
			...
			$('a.itmEdit', SStatusTable.fnGetNodes()).each(function() {
				$(this).click(function() {
					var nTr = this.parentNode.parentNode;
					var aData = SStatusTable.fnGetData(nTr);
                    RecordId = aData[0];
                    $('#RecordId').val(aData[0]);
                    $('#SStatusDesc').val(aData[2]);
					msg = "Do you really want to edit this record?";
					onCustomModal(msg, "onEditPanel");
				});
			});
			$('a.itmDrop', SStatusTable.fnGetNodes()).each(function() {
				$(this).click(function() {
					var nTr = this.parentNode.parentNode;
					var aData = SStatusTable.fnGetData(nTr);
					RecordId = aData[0];
			...
			$('a.itmMore', SStatusTable.fnGetNodes()).each(function() {
				$(this).click(function() {
					var nTr = this.parentNode.parentNode;
					if ($(this).children('span').attr('class') == 'label label-info faminus') {
						...
						SStatusTable.fnOpen(nTr, fnFormatDetails(nTr), 'details');
					}
				});
			});
		},
