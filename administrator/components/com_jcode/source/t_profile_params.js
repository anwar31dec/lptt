...
function onFormPanel() {
	resetForm("profile_param_form");
	RecordId = '';
	$("#BShow").prop("checked", true);
	checkStatus();
...
function checkStatus() {
    var checkboxstate = document.getElementById('BShow').checked;
    $('#BShow').val(checkboxstate);
}
...
function onConfirmWhenAddEdit() {
	$.ajax({
		...
		"data" : $('#profile_param_form').serialize() + '&jUserId=' + userId + '&language=' + engbId,
...
function onConfirmWhenDelete() {
	$.ajax({
		"type" : "POST",
		"url" : baseUrl + "stage_one_datasourse.php",
		"data" : 'action=deleteProfileParamData&RecordId=' + RecordId+ '&jUserId=' + userId + '&language=' + engbId,
		"success" : function(response) {
			$msgType = JSON.parse(response)['msgType'];
			$msg = JSON.parse(response)['msg'];
			if ($msgType == "success") {
...
jQuery('#profile_param_form').parsley({
	listeners : {
...
$(function() {
	...
	paramTable = $('#paramTable').dataTable({
		...
		"fnDrawCallback" : function(oSettings) {
			...
			$('a.itmEdit', paramTable.fnGetNodes()).each(function() {
				$(this).click(function() {
					var nTr = this.parentNode.parentNode;
					var aData = paramTable.fnGetData(nTr);
                    RecordId = aData[0];
                    $('#RecordId').val(aData[0]);
                    $('#ParamName').val(aData[3]);
                    $('#ParamNameFrench').val(aData[4]);
			...
			$('a.itmDrop', paramTable.fnGetNodes()).each(function() {
				$(this).click(function() {
					var nTr = this.parentNode.parentNode;
					var aData = paramTable.fnGetData(nTr);
					RecordId = aData[0];
			...
			$('a.itmMore', paramTable.fnGetNodes()).each(function() {
				$(this).click(function() {
					var nTr = this.parentNode.parentNode;
					if ($(this).children('span').attr('class') == 'label label-info faminus') {
						...
						paramTable.fnOpen(nTr, fnFormatDetails(nTr), 'details');
					}
				});
			});
		},
