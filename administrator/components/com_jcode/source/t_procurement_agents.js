...
function onFormPanel() {
	resetForm("t_procurement_agents_form");
	RecordId = '';
...
function onConfirmWhenAddEdit() {
	$.ajax({
		...
		"data" : $('#t_procurement_agents_form').serialize() + '&jUserId=' + userId + '&language=' + engbId,
		"success" : function(response) {
			$msgType = JSON.parse(response)['msgType'];
			$msg = JSON.parse(response)['msg'];
			if ($msgType == "success") {
				ProcuringAgentsTable.fnDraw();
				onSuccessMsg($msg);
				onListPanel();
			} else {
				onErrorMsg($msg);
...
function onConfirmWhenDelete() {
	$.ajax({
		"type" : "POST",
		"url" : baseUrl + "stage_one_datasourse.php",
		"data" : 'action=deleteProcuringAgentsData&RecordId=' + RecordId + '&jUserId=' + userId + '&language=' + engbId,
		"success" : function(response) {
			$msgType = JSON.parse(response)['msgType'];
			$msg = JSON.parse(response)['msg'];
			if ($msgType == "success") {
...
jQuery('#t_procurement_agents_form').parsley({
	listeners : {
...
$(function() {
	...
	$('#submitItemList').click(function() {
		$("#t_procurement_agents_form").submit();
	});
	...
	ProcuringAgentsTable = $('#ProcuringAgentsTable').dataTable({
		...
		"fnDrawCallback" : function(oSettings) {
			...
			$('a.itmEdit', ProcuringAgentsTable.fnGetNodes()).each(function() {
				$(this).click(function() {
					var nTr = this.parentNode.parentNode;
					var aData = ProcuringAgentsTable.fnGetData(nTr);
                    RecordId = aData[0];
                    $('#RecordId').val(aData[0]);
                    $('#PAgencyName').val(aData[2]);
					msg = "Do you really want to edit this record?";
					onCustomModal(msg, "onEditPanel");
				});
			});
			
			$('a.itmDrop', ProcuringAgentsTable.fnGetNodes()).each(function() {
				$(this).click(function() {
					var nTr = this.parentNode.parentNode;
					var aData = ProcuringAgentsTable.fnGetData(nTr);
					RecordId = aData[0];
