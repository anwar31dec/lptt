			...
			<div class="col-md-12 col-sm-12 col-sx-12">
				<div id="cparams-panel" class="panel panel-default">
					<div class="panel-heading clearfix" style="height: 50px;">
						<?php echo $TEXT['Facility Adjustment Report']; ?>
						...
						<div class="clearfix list-panel" id="tbl-fic">
							<table class="table table-striped table-bordered display table-hover" cellspacing="0" id="tbl-service-indicators">
								<thead>
									<tr>	
										<th>SL</th>									
										<th><?php echo $TEXT['Facility']; ?></th>								
										<th><?php echo $TEXT['Product']; ?></th>
										<th><?php echo $TEXT['Adj+']; ?></th>
										<th><?php echo $TEXT['Adj-']; ?></th>
										<th><?php echo $TEXT['Reason']; ?></th>
									</tr>
								</thead>
								<tbody></tbody>
							</table>
...
function print_function(type){
	...
	var reportSaveName = 'Facility_Stock_Adjustment'; //Not allow any type of special character of cahrtName
	...
	var reportHeaderName = TEXT['Facility Stock Adjustment']+'  '+ $('#country-list option[value='+$('#country-list').val()+']').text()+' on '+ $('#month-list option[value='+$('#month-list').val()+']').text()+',  '+ $('#year-list option[value='+$('#year-list').val()+']').text();
	reportHeaderList[0] = reportHeaderName;
	reportHeaderList[1] = TEXT['Region']+ ': ' + $('#Region-list option[value='+$('#Region-list').val()+']').text()+'  '+TEXT['District']+ ': ' + $('#District-list option[value='+$('#District-list').val()+']').text();
	reportHeaderList[1] = TEXT['Owner Type']+ ': ' + $('#OwnerType option[value='+$('#OwnerType').val()+']').text()+'  '+TEXT['Product Group']+ ': ' + $('#item-group-list option[value='+$('#item-group-list').val()+']').text();
	...
	sqlParameterList[4]= (($('Region-list').val() == '') ? 0 : $('#Region-list').val());	
	sqlParameterList[5]= (($('District-list').val() == '') ? 0 : $('#District-list').val());	
	sqlParameterList[6]= (($('OwnerType').val() == '') ? 0 : $('#OwnerType').val());	
	sqlParameterList[7]= (($('item-group-list').val() == '') ? 0 : $('#item-group-list').val());
	//groupBySqlIndex = 5;
	//colorCodeIndex[0] = 5;
	//checkBoxIndex[0] = 3;
	//checkBoxIndex[1] = 6;
  ...
  if(type == 'print'){	 
		window.open("<?php echo $baseUrl;?>report/print_master.php?action=getServiceIndicators1"
		...
		window.open("<?php echo $baseUrl;?>report/excel_master.php?action=getServiceIndicators1"
		+"&jBaseUrl=<?php	echo $jBaseUrl; ?>"
		+"&lan=<?php echo $lan;?>"
		+"&reportSaveName="+reportSaveName 
		+"&sSearch="+currentSearch 
...
<script src='<?php echo $baseUrl; ?>/report_adjustment.js'></script>
