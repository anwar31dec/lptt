...
//[Popup Link]
if(!function_exists('azurapopuplink_sc')) {

	function azurapopuplink_sc( $atts, $content="" ) {
    ...
    if(!empty($styleTextTest)){
      $popuplinkstyle .= trim($styleText);
    }

    if(!empty($popuplinkstyle)){
      $popuplinkstyle = 'style="'.$popuplinkstyle.'"';
    }
		...
		if(stripos($layout, '_:') !== false){
			...
			if(stripos($layout, ':') !== false){
				...
				$shortcodeTemp = CthShortcodes::addShortcodeTemplate('azurapopuplink');
			}
		}
	...
	add_shortcode( 'AzuraPopupLink', 'azurapopuplink_sc' );
}